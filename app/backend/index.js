...
app.post("/api/adv1", (require, response) => {
  ...
  const ratingsCount = require.body.ratingsCount;
  const restPrice = require.body.restPrice;
  const sqlInsert = "(SELECT RecipeID, RecipeName, CookingTime, Rating, ServingSize FROM Recipes WHERE CookingTime < ? AND Rating > ? AND RatingsCount > ?) UNION (SELECT RecipeID, RecipeName, CookingTime, Rating, ServingSize FROM Recipes NATURAL JOIN Serves JOIN Restaurants USING (RestaurantID) WHERE Rest_Price < ?) LIMIT 10;" ;
  pool.query(sqlInsert, [cookingTime, rating, ratingsCount, restPrice], (err, result) => {
    if (err) {
      response.send(err);
    } else {
...
app.post("/api/adv2", (require, response) => {
  const difference = require.body.difference;
  const sqlInsert = "SELECT DISTINCT RecipeID, CookingTime, RecipeName, RestaurantName, Rest_Price, Round(STOREPRICE, 2) AS store_Price, Rating, ServingSize FROM Serves NATURAL JOIN Recipes JOIN Restaurants USING (RestaurantID) JOIN (SELECT RecipeID, SUM(Store_Price) as STOREPRICE FROM Recipe_map NATURAL JOIN Buy JOIN Store USING (StoreID) GROUP BY RecipeID) T1 USING (RecipeID) WHERE ABS(Rest_Price-STOREPRICE ) <= ? LIMIT 15;" ;
  pool.query(sqlInsert, [difference], (err, result) => {
    if (err) {
      response.send(err);
    } else {
